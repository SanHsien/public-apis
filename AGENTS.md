...
- 一般變更直接推 `origin/master`，不開功能分支、不開維護 PR（2026-08-22 起）。只有在需要他人審查、或改動風險高到值得先讓 CI 在 PR 上跑一輪時，才退回 **branch → PR → CI → merge**。與 `CLAUDE.md`、`FORK.md` 一致。合併任何 PR（含 Dependabot）前仍必須讀完整 diff。
